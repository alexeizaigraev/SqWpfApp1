...
namespace SqWpfApp1
{
    class Priem : PeoplePapa
    {
        ...
        internal static string agentSign = "";

        //private List<List<string>> comonData = DbComonDataMeth.GetAllComonData();
        public static void MainPriem()
        {
            ...
            foreach (string line in MkGoodArr(lines))
            {
                ...
                agentSign = Terminal().Substring(0, 3);
                string agent = DbBase.GetList($@"SELECT kass_owner
FROM comon_data
WHERE code = '{agentSign}';")[0];
                string mail = MkMail();
        ...
        private static string MkMail()
        {
            ...
            if (
                ...
                mail = DbBase.GetList($@"SELECT email
FROM comon_data
WHERE code = '{agentSign}';")[0];
